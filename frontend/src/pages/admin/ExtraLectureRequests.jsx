...
export default function ExtraLectureRequests() {
  ...
  const load = async () => { try { const response = await api.get("/extra-lecture-requests"); setRequests(response.data.requests || []); } catch (e) { setError(e.response?.data?.message || "Failed to load requests."); } };
  useEffect(() => { load(); }, []);
  const review = async (request, status) => { const adminNote = window.prompt(status === "approved" ? "Optional approval note:" : "Optional rejection reason:", ""); if (adminNote === null) return; try { const response = await api.patch(`/extra-lecture-requests/${request._id}/review`, { status, adminNote }); setMessage(response.data.message); load(); } catch (e) { setError(e.response?.data?.message || "Could not review request."); } };
